    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <Card className="border-gray-200 dark:border-gray-800">
        <CardHeader>
          ...
          <CardDescription className="text-sm sm:text-base">Manage your automated email response templates</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 sm:space-y-4">
          {templates.map((template) => (
            <div
              key={template.id}
              className="border border-gray-200 dark:border-gray-800 rounded-lg p-3 sm:p-4 hover:bg-muted/50 transition-colors"
            >
              ...
              <p className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Subject: {template.subject}</p>
              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-3">{template.preview}</p>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs sm:text-sm text-gray-500 dark:text-gray-400 space-y-1 sm:space-y-0">
      ...
      <Card className="border-gray-200 dark:border-gray-800">
        <CardHeader>
          ...
          <CardDescription className="text-sm sm:text-base">Latest automated email responses sent by the system</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 sm:space-y-3">
          {[
            ...
            <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-muted/50 rounded-lg space-y-2 sm:space-y-0">
              <div className="flex items-center space-x-3">
                <Mail className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 flex-shrink-0" />
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate">{reply.email}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Template: {reply.template}</p>
                </div>
              </div>
              <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">{reply.time}</span>
            </div>
          ))}
        </CardContent>
      </Card>
